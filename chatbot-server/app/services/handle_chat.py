...
async def handle_chat(req: ChatRequest):
    ...
    if phone_plan_step > 0:
        ...
        return await get_multi_turn_chain(req, "subscription_multi")

    # 새로운 대화 시작 - 인텐트 감지
    intent = await detect_intent(req.message)
    print(f"[DEBUG] New conversation - detected intent: {intent}")

    # 멀티턴 인텐트들은 get_multi_turn_chain 사용
    if intent in ["phone_plan_multi", "subscription_multi"]:
        return await get_multi_turn_chain(req, intent)
